...
!###########################################################
!###########################################################
subroutine get_threetondim_nbor_grid(s,hash_key,igrid_nbor,flush_cache,fetch_cache)
  ...
  integer::i1,j1,k1,idim,ilevel,inbor,igridp
  integer(kind=8)::hash_nbor(0:ndim)

  associate(r=>s%r,g=>s%g,m=>s%mdl%m)
    ...
    do k1=-1,1
      do j1=-1,1
        do i1=-1,1
          ...
          call get_grid(s, hash_nbor, igridp, flush_cache=flush_cache, fetch_cache=fetch_cache, lock=.true.)
          igrid_nbor(inbor) = igridp
        end do
      end do
    end do

  end associate
end subroutine get_threetondim_nbor_grid
!###############################################################
!###############################################################
!###############################################################
!###############################################################
subroutine get_intermediate_nbor_grid(s,hash_key,igrid_nbor,flush_cache,fetch_cache)
  use amr_parameters, only: ndim, threetondim
  use ramses_commons, only: ramses_t
  use hash
  implicit none
