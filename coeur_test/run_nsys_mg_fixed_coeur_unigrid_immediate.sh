#!/bin/bash -l
#SBATCH --job-name=nsys-mg-coeur-now
...
run_root="runs/${run_id}_nsys_mg_fixed_coeur_unigrid_immediate"
...
(
    ...
    nsys profile --force-overwrite=true --trace=cuda,nvtx,osrt --sample=none --cpuctxsw=none \
        --output=mg_fixed_coeur_unigrid ../../../../bin/ramses3d "${namelist}" \
        > ../../../mg_nsys_fixed_coeur_unigrid_immediate_run.log 2>&1
)
mg_status=$?
